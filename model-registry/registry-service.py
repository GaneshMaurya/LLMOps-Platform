...
def validate_meta_json(meta_content: str) -> Tuple[bool, Dict[str, Any]]:
    ...
    try:
        ...
        required_fields = ["setup_commands", "start_command"]
        ...
        if not isinstance(meta_data["start_command"], str):
            return False, {"error": "start_command must be a string"}
            
        if "app.py" not in meta_data["start_command"]:
            return False, {"error": "start_command should run app.py"}
...
def run_validation(
    ...
    is_valid = all([meta_result[0], app_result[0], req_result[0], model_result[0]])
...
def main():
    uvicorn.run("registry-service:app", host='0.0.0.0', port=8000, reload=True)
